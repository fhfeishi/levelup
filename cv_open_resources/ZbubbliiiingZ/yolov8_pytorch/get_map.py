...
from yolo_for_infer import YOLO_INFER
...
if __name__ == "__main__":
    ...
    #--------------------------------------------------------------------------------------#
    MINOVERLAP      = cfg['getmap_cfg']['minoverlap']
    #--------------------------------------------------------------------------------------#
    ...
    #--------------------------------------------------------------------------------------#
    confidence      = cfg['getmap_cfg']['confidence']
    #--------------------------------------------------------------------------------------#
    #   预测时使用到的非极大抑制值的大小，越大表示非极大抑制越不严格。
    #   
    #   该值一般不调整。
    #--------------------------------------------------------------------------------------#
    nms_iou         = cfg['getmap_cfg']['nms_iou']
    #---------------------------------------------------------------------------------------------------------------#
    ...
    #---------------------------------------------------------------------------------------------------------------#
    score_threhold  = cfg['getmap_cfg']['score_threhold']
    #-------------------------------------------------------#
    #   map_vis用于指定是否开启VOC_map计算的可视化
    #-------------------------------------------------------#
    map_vis         = cfg['getmap_cfg']['map_vis']
    #-------------------------------------------------------#
    #   指向VOC数据集所在的文件夹
    #   默认指向根目录下的VOC数据集
    #-------------------------------------------------------#
    VOCdevkit_path  = cfg['getmap_cfg']['data_root'] 
    # default: VOCdevkit_path  = 'VOCdevkit'
    if VOCdevkit_path == 'VOCdevkit':
        image_ids = open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Main/test.txt")).read().strip().split()
        image_dir = os.path.join(VOCdevkit_path, "VOC2007/Annotations/")
        label_dir = os.path.join(VOCdevkit_path, "VOC2007/JPEGImages/")
    else:
        assert os.path.isdir(VOCdevkit_path), "test_data path is not exist"
        struct_ = {'iamge': 'images', 'label': 'labels'}
        image_dir = os.path.join(VOCdevkit_path, struct_['image'])
        label_dir = os.path.join(VOCdevkit_path, struct_['label'])
        image_ids = [f.split('.')[0] for f in os.listdir(image_dir)]
    #-------------------------------------------------------#
    #   结果输出的文件夹，默认为map_out
    #-------------------------------------------------------#
    map_out_path    = cfg['getmap_cfg']['map_out_path']
    ...
    if map_mode == 0 or map_mode == 1:
        print("Load model.")
        yolo = YOLO_INFER(confidence = confidence, nms_iou = nms_iou)
    ...
    if map_mode == 0 or map_mode == 2:
        ...
        for image_id in tqdm(image_ids):
            with open(os.path.join(map_out_path, "ground-truth/"+image_id+".txt"), "w") as new_f:
                root = ET.parse(os.path.join(label_dir, "VOC2007/Annotations/"+image_id+".xml")).getroot()
