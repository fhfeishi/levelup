...
# from image_pre import ImageBlock
# from image_pre import shared_params, pilimg_resize, pilimg_padded_crop
# from image_pre import pil_img
...
def merge_results(results, image_shape, crop_size, overlap):
    ...
    while y < height:
        ...
        while x < width:
            ...
            xmax = x + crop_size
            ymax = y + crop_size
            confidence = results[result_index][4]
            class_id = results[result_index][5]
            merged_result.append([xmin, ymin, xmax, ymax, confidence, class_id])
            x += crop_size - overlap
            result_index += 1
        y += crop_size - overlap
    return merged_result

def update_crops_box(box, row_index, col_index, stride_height, stride_width, original_image_size):
    """
    将图像块中的边界框坐标转换为原始高分辨率图像的坐标。

    :param box: 边界框，格式为 [top, left, bottom, right]
    :param row_index: 行索引
    :param col_index: 列索引
    :param stride_height: 在高度上的滑动步长
    :param stride_width: 在宽度上的滑动步长
    :param original_image_size: 原始图像的尺寸，格式为 (width, height)
    :return: 转换后的边界框坐标
    """
    top, left, bottom, right = box

    # 将坐标转换为原始图像的坐标系
    original_top = max(0, np.floor(top).astype('int32')) + row_index * stride_height
    original_left = max(0, np.floor(left).astype('int32')) + col_index * stride_width
    original_bottom = min(original_image_size[1], np.floor(bottom).astype('int32')) + row_index * stride_height
    original_right = min(original_image_size[0], np.floor(right).astype('int32')) + col_index * stride_width

    return [original_top, original_left, original_bottom, original_right]

def crops_iou(box1, box2):
    """
        计算两个边界框的交并比。
...
def crops_nms(boxes, scores, iou_threshold=0.3):
    if len(boxes) == 0:
        return []
...
def nms_with_merging(results_record, iou_threshold=0.4, merge_threshold=0.05):
    """
    对多个类别执行NMS，并合并相邻的边界框

    :param results_record: 包含边界框、置信度和类别的字典列表
    :param iou_threshold: 交并比阈值
    :param merge_threshold: 合并阈值，用于判断是否合并两个边界框
    :return: 抑制后的结果
    """
    ...
    def nms_single_class(boxes):
        """
        对单个类别执行NMS，并在必要时合并边界框
        """
        if len(boxes) == 0:
            return []
        ...
        while boxes:
            ...
            merged = False
            for i, other_box in enumerate(keep):
                if compute_iou(base_box['box'], other_box['box']) > merge_threshold:
                    new_box = merge_boxes(base_box['box'], other_box['box'])

                    keep[i] = {
                        'box': new_box,
                        'score': max(base_box['score'], other_box['score']),
                        'label': base_box['label'],
                        'color': base_box['color'],
                        'font': base_box['font']
                    }
                    merged = True
                    break
    ...
    for record in results_record:
        cls = record['label']
        boxes_per_class.setdefault(cls, []).append(record)

    # 对每个类别执行NMS
    nms_results = []
    for cls, boxes in boxes_per_class.items():
        nms_results.extend(nms_single_class(boxes))

    return nms_results

# 使用 nms_results = nms_with_merging(results_record, iou_threshold=0.4, merge_threshold=0.5) 调用这个函数
