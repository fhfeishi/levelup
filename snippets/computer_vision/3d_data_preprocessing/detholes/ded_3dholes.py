...
class det3d_holes():
    ...
    def pipeline(self, tgt_path="24_detpipe.ply"):
        ...
        # 1. 粗拟合平面去噪 0.5
        pcd, _ = self.plane_filter(max_distance=0.7)
        # # 2. 精确拟合平面
        pcd, plane_model = self.plane_filter(pc=pcd, max_distance=0.2)
        # # 3. 体素降采样
        pcd = self.mtd_voxel(pc=pcd, voxel_size=0.24)
        # # 4. 根据密度找到边沿
        pcd = self.get_edgesPCD(pc=pcd, max_r=2.0, neighbor_thresh=(70, 235))
        # # 5. 提取出目标孔洞
...
if __name__ == "__main__":
    ...
    holes3d.pipeline(tgt_path="24_detpipe123456.ply")
