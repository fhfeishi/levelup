...
class pil_img:
    ...
    # slide window for object detec
    def paddedImageCrop_obj(self, image, crop_size=(640, 640), overlap=(0, 0), show_rows_cols_num=False, show_padding_wh=False,plot=False, all_box_show=False,
            ...
            # 新建pillow画布 1------  全显示的分割框， 验证代码是不是写好
    ...
    # slide window for semantic segmentation  # input: PIL.Image.Image
    def paddedImageCrop_seg(self, image, crop_size=(640,640), overlap=(0, 0)):
            width, height = image.size
            self.crop_width, self.crop_height = crop_size
...
# 推理的时候把一张图片划分为了多个block， 然后把这一堆block<'class' ImageBlock>作为一个batch传到model，用gpu并行来加速
# 肯定输出的也是一个class  OutBlock
# 
